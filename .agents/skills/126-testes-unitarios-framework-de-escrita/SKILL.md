---
name: 126-testes-unitarios-framework-de-escrita
description: Planeja e escreve testes unitários para o Renda Comparada quando uma mudança exigir cenários de comportamento, casos-limite, erros e proteção contra regressões.
---
...
Linguagem: TypeScriptFramework: JestCódigo: Função calculateDiscount(price, couponCode, userTier)
...
describe('calculateDiscount', () => { describe('sem cupom', () => { it('retorna preço original para usuário regular sem cupom', () => { const result = calculateDiscount(100, null, 'regular'); expect(result).toBe(100); }); it('aplica 10% de desconto para usuário premium', () => { const result = calculateDiscount(100, null, 'premium'); expect(result).toBe(90); }); it('aplica 20% de desconto para usuário VIP', () => { const result = calculateDiscount(100, null, 'vip'); expect(result).toBe(80); });

});

describe('edge cases', () => { it('nunca retorna valor menor que R$1.00', () => { const result = calculateDiscount(5, 'SAVE99', 'vip'); expect(result).toBe(1); }); it('lança erro para preço negativo', () => { expect(() => calculateDiscount(-10, null, 'regular')) .toThrow('Preço deve ser positivo'); });
